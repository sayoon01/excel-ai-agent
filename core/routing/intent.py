...
_INTENT_MAP: dict[str, list[str]] = {
    "filter":    ["필터", "뽑아", "추출", "조건", "이상", "이하", "초과", "미만",
                  ...
                  "포함", "포함된", "들어간", "들어있", "들어가",
                  "비어있", "비어 있", "비었", "공백",
                  "더 큰", "더 많은", "더 높은", "더 작은", "더 적은", "더 낮은",
                  "보다 큰", "보다 작은", "보다 많은", "보다 적은",
                  "제외", "where", "해당", "특정", "걸러", "골라"],
    ...
    # 주의: "행", "열"은 너무 광범위해 filter/aggregate 문장을 침범 → 제거.
    #       "행 수"·"몇 행"은 task_router의 get_row_count 룰이 더 강하게 잡아냄.
...
_MERGE_JOIN_HINTS: list[str] = [
    ...
    # ID 키워드(사번/고객id/주문번호 등)는 단독으로 join 의도가 아니므로 제외.
    # 다른 의도(필터·정렬·집계)에서 같은 컬럼명을 언급하는 케이스를 망가뜨림.
]
